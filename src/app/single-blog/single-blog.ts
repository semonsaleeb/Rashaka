...
ngOnInit(): void {
  ...
  if (id) {
    this.blogService.getBlogById(id).subscribe({
      next: (data) => {
        this.blog = data;
        this.updateBlogHtml(); // initial render
  ...
  this.languageService.currentLang$.subscribe(lang => {
    ...
    this.updateBlogHtml(); // update content dynamically
  });
}
...
updateBlogHtml(): void {
  if (!this.blog) return;
  this.blogHtml = this.parseContent(
    this.currentLang === 'ar' ? this.blog.content_ar : this.blog.content
  );
}
